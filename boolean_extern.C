// boolean_extern.C -- Extract a single boolean from an extern log.
// 
...
struct BooleanExtern : public Boolean
{
  ...
  bool value (const Scope& inherit_scope) const
  { 
    daisy_assert (extern_scope);
    ScopeMulti scope (*extern_scope, inherit_scope);
...
Name of extern log to fecth values from.");
    ...
    Librarian<Boolean>::add_type ("extern", alist, syntax, &make);
  }
} BooleanExtern_syntax;

// boolean_extern.C ends here
