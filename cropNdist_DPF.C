...
static const double Mw = 14.0067; //The molecular weight for N [g mol¯1]
				  ...
				  const double cropN /*[g/m²area]*/, Treelog&)
{
  ...
  for (int i = 0; i < No; i++)
     cropNdist[i] = f_photo * cropN0 * (exp(-kn * dLAI *i)); //[mol/m² leaf]
		       ...
		       std::vector <double>& cropVm)
{
  ...
  for (int i = 0; i < No; i++)
     cropVm[i] = Xn / 1000.0 * cropNdist[i]; //[mol/m² leaf/s]
}
...
static struct cropNdistDPFSyntax
{
  ...
  static void load_syntax (Syntax& syntax, AttributeList& alist)
  {
    ...
    syntax.add ("Xn", "mmol/mol/s", Check::positive (), Syntax::Const,
                "Slope of relationship between leaf nitrogen and Vmax, Xn = 1.16 for wheat (de Pury & Farquhar, 1997)");
    alist.add ("Xn", 1.16);
  }  
  cropNdistDPFSyntax ()
  {
    ...
    alist.add ("description", 
	       "De Pury & Farquhar (1997) crop N-distribution model in the canopy for photosynthesis and distribution of photosynthetical capacity.");


    load_syntax (syntax, alist);

    Librarian<CropNdist>::add_type ("DPF", alist, syntax, &make);
  }
} cropNdistDPF_syntax;
