...
class BioclimatePM : public Bioclimate
{ 
  // Canopy.
public:
...
public:
  ...
  // Create.
public:
  BioclimatePM (const AttributeList&);
  ~BioclimatePM ()
    { }
};

BioclimatePM::BioclimatePM (const AttributeList& al)
...
void 
BioclimatePM::RadiationDistribution (const Weather& weather, 
...
void
BioclimatePM::IntensityDistribution (const double Rad0,
					   const double Ext,
					   vector <double>& Rad) const
{
  double dLAI = (LAI_ / No);
...
void
BioclimatePM::WaterDistribution (Surface& surface,
...
void 
BioclimatePM::tick (Surface& surface, const Weather& weather, 
			  const Time&,
			  const CropList& crops, const Soil& soil, 
...
void 
BioclimatePM::output (Log& log, Filter& filter) const
{
...
void
BioclimatePM::irrigate (double flux, double temp, 
...
static struct BioclimatePMSyntax
{
  static Bioclimate& make (const AttributeList& al)
    { return *new BioclimatePM (al); }
  ...
  BioclimatePMSyntax ()
    {
      ...
      Librarian<Bioclimate>::add_type ("PM", alist, syntax, &make);
    }
} BioclimatePM_syntax;
