...
bool 
ActionCrop::MM_DD::check_alist (const AttributeList& alist)
{
  bool ok = true;

  const int mm = alist.integer ("month");
  const int dd = alist.integer ("day");
  const int hh = alist.check ("hour") ? alist.integer ("hour") : 8;
...
ActionCrop::MM_DD::load_syntax (Syntax& syntax, AttributeList& alist)
{
  ...
  syntax.add ("day", Syntax::Integer, Syntax::Const, 
	      "Day in the month.");
  syntax.add ("hour", Syntax::Integer, Syntax::OptionalConst, 
	      "Hour in the day.");
  alist.add ("hour", 8);
...
ActionCrop::MM_DD::MM_DD (const AttributeList& al)
  : month (al.integer ("month")),
    day (al.integer ("day")),
    hour (al.check ("hour") ? al.integer ("hour") : 8)
{ }
...
bool 
ActionCrop::Sow::check_alist (const AttributeList&)
{
  bool ok = true;
  return ok;
}
...
{ 
  ...
  syntax.add ("done", Syntax::Boolean, Syntax::OptionalState, 
...
ActionCrop::Sow::Sow (const AttributeList& al)
  : date (al.alist ("date")),
    crop (al.alist ("crop")),
    done (al.check ("done") && al.flag ("done"))
{ }
...
bool 
ActionCrop::Perennial::check_alist (const AttributeList& al)
{
  bool ok = true;

  if (al.integer ("seasons") < 1)
    {
      CERR << "Perennial harvest should last at least 1 season\n";
      ok = false;
    }
  non_negative (al.number ("DS"), "DS", ok);
  non_negative (al.number ("DM"), "DM", ok);
  if (al.check ("fertilize_rest") && !al.check ("fertilize"))
    {
      CERR << "`fertilize_rest' specified, but `fertilize' isn't\n";
      ok = false;
    }
...
First season is defined as the year where the first harvest occurs.");
  syntax.add ("fertilize_index", Syntax::Integer, Syntax::OptionalState,
...
If missing, use the same fertilizer as first season.");
  syntax.add ("fertilize_rest_index", Syntax::Integer, Syntax::OptionalState,
...
ActionCrop::Perennial::Perennial (const AttributeList& al)
  : seasons (al.integer ("seasons")),
    ...
    fertilize (al.check ("fertilize")
	       ...
	       : NULL),
    fertilize_index (al.check ("fertilize_index") 
		     ? al.integer ("fertilize_index")
		     : 0),
    fertilize_rest (al.check ("fertilize_rest")
		    ? &al.alist_sequence ("fertilize_rest")
		    : NULL),
    fertilize_rest_index (al.check ("fertilize_rest_index")
			  ? al.integer ("fertilize_rest_index")
			  : 0),
...
bool 
ActionCrop::Fertilize::check_alist (const AttributeList& al)
{
  bool ok = true;
...
bool 
ActionCrop::Tillage::check_alist (const AttributeList& al)
{
  bool ok = true;
  const int mm = al.integer ("month");
  const int dd = al.integer ("day");
...
bool 
ActionCrop::Irrigation::check_alist (const AttributeList& al)
{
  bool ok = true;
  non_negative (al.number ("amount"), "amount", ok);
  non_positive (al.number ("potential"), "potential", ok);
  return ok;
}
...
ActionCrop::ActionCrop (const AttributeList& al)
  : Action (al),
    ...
    irrigation_rest (al.check ("irrigation_rest") 
		     ...
		     : NULL),
    irrigation_year (al.integer ("irrigation_year")),
    irrigation_delay (al.integer ("irrigation_delay"))
{ }
...
static struct ActionCropSyntax
{
  ...
  static bool check_alist (const AttributeList& al)
  {
    ...
    if (al.check ("irrigation_rest") && !al.check ("irrigation"))
      {
	CERR << "`irrigation_rest' specified, but `irrigation' isn't\n";
	ok = false;
      }
    if (!al.check ("harvest_annual") && !al.check ("harvest_perennial"))
      {
	CERR << "No harvest specified\n";
	ok = false;
      }
    if (al.check ("harvest_annual") && al.check ("harvest_perennial")
	&& !al.check ("secondary"))
      {
	CERR << "You must specify a secondary crop when you specify both "
  ...
  { 
    ...
    add_submodule<ActionCrop::Sow>("primary", syntax, alist, Syntax::State,
				   "Primary crop.");
    add_submodule<ActionCrop::Sow>("secondary", syntax, dummy,
				   Syntax::OptionalState, 
				   "Secondary crop, if any.");
    add_submodule<ActionCrop::Annual>("harvest_annual", syntax, dummy,
				      Syntax::OptionalState,
				      "Harvest parameters for annual crops.");
    add_submodule<ActionCrop::Perennial>("harvest_perennial", syntax, dummy,
					 Syntax::OptionalState, "\
