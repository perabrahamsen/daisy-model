...
non-overlapping rectangles (Figure \ref{fig:grid_rect}).
...
The cells are denoted $Q_i$ where $i=1,2,\cdots, N$. $|Q_i|$ denotes
the area of $Q_i$, and $\partial Q_i$ is the boundary of $Q_i$
i.e. the edges (or faces) of $Q_i$. All internal edges $e_{ij}$ are
labeled by indices, $i$ and $j$ of the adjacent cells that shares
face. The grid is constructed such that only whole faces are shared
($e_{ij}=Q_i \cap Q_j$). The length of $e_{ij}$ is $|e_{ij}|$ and the
unit normal vector pointing from $Q_i $into $Q_j$ and orthogonal to
$e_{ij}$ is denoted $\bar{\mathbf{n}}_{ij}$. $\sigma_i$ contains cell
indices of cells sharing faces with cell $i$. $\sigma_i'$ contain
indices of cell faces of cell $i$ which are placed on $\partial
\Omega$, i.e. it is not shared with another cell.  $\sigma_i'$ is
divided into two subsets, $\sigma_{i}'^{D}$ and $\sigma_{i}'^{N}$ of
boundary cell faces with a Dirichlet and Neumann boundary condition,
respectively.
...
%%$(\nabla \psi)_{ij}$ is evaluated by a different method for meshes
%%with rectangular cells than for the more general and complicated case
%%with meshes consisting of trapezoid cells.
The gravitational transport from cell $i$
...
\rightarrow \text{1}}$.
%
%% XXXX Soren er der noget fornuftigt at sige her ? XXXX
...
diffusion equation is considered as linear inside each timestep. I.e.
the coefficients in the equation in each of the timesteps are
...
dependent only on the concentrations in the previous timestep.
Similar are adsorption added as sink/source term. Thus different
from the water movement simulation, the Picard iterations loop
used inside each timestep can be avoided.
