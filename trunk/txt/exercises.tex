...
Søren Hansen and Per Abrahamsen\\
...
the value 15.5 is used to give the dimension of the number. In the
...
computational environment. The working folder is by default the folder
where the \daisy{}-setup-file is stored. However, output files can be routed 
to a sub-folder by use of of the command \verb|(log_prefix "E1/")| (see below). 
At installation \Daisy{} stores external library-files in prededined folders, 
where \Daisy{} by default will search for the information. Box~\ref{box:header}
shows how external library-files including the weather file are retrived by 
the model.
When the \daisy{} code is runnig it writes a log-file (\texttt{daisy.log}), where
all messages from the simulation, including error messages, is stored in the 
working folder.
...
external file, which then could be included by a input file
...
shown in box~\ref{box:activation} we activate a column, select a stating 
date and an ending date, and we select an already defined agricultural 
management practice.
...
First we write a description of the simulation that will appear in 
all output files. Then we tell the model to store the output files 
in a sub-folder to the actual working folder. This is done by the 
command \verb|(log_prefix "E1/")|, which will place all output files in
the subfolder (\texttt{E1}).  You must create the sub-folder before
...
The only exception from the latter is the \texttt{harvest.dlf}-file; 
all other output files can be viewed by the
...
\texttt{Exercise01.dai} and run a simulation either  by activating
the model within TextPad (go to Tools and select \daisy{}) or by dragging 
and dropping the setup-file at the \daisy{} executeable. Inspect the created
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lllllllll}\hline
    ...
    cm & (h=0cm) & 1.0 & 1.2 & 1.7 & 2.0 & 2.2 & 2.7 & 4.2\\\hline
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{llrrrr}                              \hline
    ...
    cm     & (h=0cm)    & 1.0   & 1.2  & 1.4  & 1.7  \\\hline
...
\begin{verbatim}
  (Drain lateral)
  (Groundwater aquitard (K_aquitard 0.01 [cm/h]))
\end{verbatim}
instead of 
\begin{verbatim}
  (Groundwater deep)
...
;; application (defam = defining added matter)
...
content, which was defined in the horizon section
(box~\ref{box:horizon}).  The MIT model comprises several organic
matter pools and the total carbon content is distributed among these
pools by the initialization procedure that makes use of annual input of
...
inactive pool.  This is done be entering the following parameter in
...
\texttt{AArslevWeatherDat.xls}. Årslev is a locality located at the
...
79\% of the C with a half-life of 150 days and the easier
...
The columns for the main log files.
