...
%%\documentclass[preprint,review,1p,authoryear,12pt]{elsarticle}
\usepackage{mathptmx}
...
\author[ku]{Søren Hansen}

\cortext[cor]{Corresponding author at: Department of Basic Sciences and Environment, Faculty of Life Sciences, University of Copenhagen, Thorvaldsensvej 40, DK-1871, Frederiksberg, Denmark; Tel.: +45-353-33393; fax: +45-353-33300}
...
\begin{abstract}
  ...
  distribution. The objective of this paper is to present a simple and
  robust model of root density distribution for row crops, based on
  physical quantities that can be estimated by a plant growth model.
  ...
  mass, root zone width, and root zone depth. Root density for
  potatoes and tomatoes were measured at two sites.  Both the original
  and the extended model are fitted to these datasets. An F-test is
  used to determine if the extended model is significantly better than
  the original model.  The extended model is superior in the cases
  where the root zone width is less than the distance between rows.
  The median of the coefficient of determination (R$^2$) for all the
  ...
  variation.  Our simple model thus was able to adequately describe
  the measured data, relying on the three quantities listed above, all
...
With alternate partial root zone drying (PRD) irrigation,
...
with depth to capture the field observations.  These root models are
parametric, with the roots described as a density function, e.g.\ $L\,
...
quantities. The model will be tested on two independent sets of
...
processes of the specific setup used in SAFIR are depicted on
...
any of the previous papers, nor has the root density measurements from
...
\begin{figure}[htbp]
  ...
  \caption{The root density model (red) in the context of Daisy as
    ...
    uptake is then calculated based on the hydraulic conductivity
    ($K$), the pressure head ($h$), and the nitrogen solute
    concentration ($C_N$) of nitrate and ammonium.  The later three
    are delivered by the soil module, which receive sink terms for
    water and nitrogen ($S_\Theta$ and $S_N$) in return. The crop
    growth is mainly driven by assimilated CO$_2$ from photosynthesis,
    which is calculated by the SVAT module, based on weather data,
    actual transpiration, and the leaf area (LAI) and nitrogen status
    of leafs (Leaf-N).}
  \label{fig:daisy}
\end{figure}
...
root length $S_r$ is a known constant $l_r = S_r \, M_r$.
...
\begin{equation}
  ...
  \label{eq:Lambert}
\end{equation}
If the root length is suffiently high the equation will have two
...
$W = -2$.  After finding a value for $W$, we can find the desired density
parameters $a = -W / d_c$ and $L_0 = L_m e^{a \, d_c}$.
...
assume as before that the root zone depth and root mass are known, and now
...
\end{eqnarray}
Thus $e^{-a_z d_c} = e^{-a_x w_c}$ or $a_x = a_z d_c / w_c $.
By inserting this in \MyEq{eq:Ld0} we get
\begin{equation}
  L_m = l_R\, a_z\, \frac{d_c}{w_c} a_z \, e^{-a_z d_c}
  \label{eq:azeq1}
\end{equation}
If we substitute $Q = -a_z d_c$ and isolate the known values on the
right side, this gives us:
\begin{equation}
  Q^2\, e^Q = L_m \frac{d_c \, w_c}{l_R}
  \label{eq:logsquare}
\end{equation}
The equation can have three solutions, using reasoning similar to
...
using Newton's method with an initial guess of $Q = -3$. Knowing $Q$ we
can find $a_z$, $a_x$, and $L_{0,0}$.
...
being operated by the Chinese Academy of Agricultural Sciences (CAAS)
and located in Xinxiang city in the Henan province, 600 km SW of
Beijing, and the second operated by the Chinese Agricultural
University and located near Beijing.  Potatoes (\textit{Solanum
  tuberosum} L. cv. 'Zheng No.5') were grown at the CAAS site,
while the CAU site were growing tomatoes (\textit{Lycopersicon esculentum} Mill. cv. 'Hong-fen').
...
and furrow irrigation.  The irrigation strategies were full irrigation
(equivalent to potential evapotranspiration), deficit irrigation (70\%
of full irrigation), and partial rootzone drying (like deficit, but
application on alternating sides of the crop).
...
24	& 	& Furrow	& Full irrigated\\\hline
  \end{tabular}
\end{table}

\begin{table}[htbp]
  \caption{CAU treatments with root density measurements.  
    All treatments used tap water.}
  \label{tab:cautreatments}
  \begin{tabular}{llllll}\\\hline
\multicolumn{3}{c}{2007} & \multicolumn{3}{c}{2008}\\
Plot	& Method	& Strategy	& Plot	& Method	& Strategy\\\hline
1	& Subsurface	& Partial rootzone drying	& 3	& Subsurface	& Partial rootzone drying\\
2	& Subsurface	& Partial rootzone drying	& 6	& Subsurface	& Full irrigation\\
3	& Subsurface	& Partial rootzone drying	& 9	& Furrow	& Partial rootzone drying\\
4	& Subsurface	& Full irrigation	& 12	& Furrow	& Full irrigation\\
5	& Subsurface	& Full irrigation	& 15	& Subsurface	& Full irrigation\\\hline
\end{tabular}
\end{table}
...
in a $3\times2\times7$ grid of sample points.  The sampling along the
x-axis corresponded to the top of the ridge, the middle of the ridge
wall, and the bottom of the valley ($x$ = 0 cm, $x$ = 18.75 cm, $x$ =
37.5 cm).  The sampling along the y-axis corresponded to the plant
...
Fig.~\ref{fig:sample-caas}. Trenches were dug at the two y-positions
to app.\ 1 m depth along the x-axis. Then soil cores were extracted
with 6.5 x 15 cm stainless steel cylinders hammered horizontally into
the trench side..  At the CAU site, the sampling was done done as the
CAAS site by digging a trench and extracting soil soil cores with a
size of 8 x 10 cm length. The sampling was here in a cross shape, with
five sample locations at the top of the ridge ($x$ = 0 cm; and $y$ =
-20 cm, $y$ = -10 cm, $y$ = 0 cm, $y$ = 10 cm, $y$ = 20 cm), and five
sample locations across the ridge system ($y$ = 0 cm; and $x$ = -30
cm, $x$ = -15 cm, $x$ = 15 cm, $x$ = 0 cm, $x$ = 30 cm).  The sampling
along the top of the ridge started at $z$ = -15 cm, that is above
undisturbed ground level.  The sampling across the ridge started at
$z$ = 5 cm.  The samplings were performed in 10 cm intervals, the
lowest varied between plots and with the horizontal distance to the
plant, the lowest were at $z$ = 55 cm. See Fig.~\ref{fig:sample-cau}.
...
\begin{table}[htbp]
  \caption{The best fit for root zone depth ($d_c$),
    ...
    for each plot at CAAS.  The `All' plot indicates a dataset containing all
...
24	& 132	& 164	& 0.74	& 0.73	& 0.69	& 4.10	& 4.14\\\hline
  \end{tabular}
\end{table}
...
\begin{table}[htbp]
  \caption{The best fit for root zone depth ($d_c$),
    root zone diameter ($2 w_c$), and total root dry matter ($M_r$) is show
    for each plot at CAU.  The `All' plot indicates a dataset containing all
    the individual plots.  Furthermore, the R$^2$ for both the \oned{}
    and \twod{} fits are listed, as well as an F test indicating whether
    the \twod{} model provides significantly better fit (\textbf{bold}).} 
  \label{tab:ftest-cau}
...
15	& 113	& 47	& 0.26	& 0.89	& 0.11	& \textbf{101.73}	& 4.60\\\hline
  \end{tabular}
\end{table}
...
generalized version of a well know and tested \oned{} model. To test
the new model, we have fitted both the original \oned{} model and the
generalized \twod{} model to datasets spanning two years data from two
sites (CAAS and CAU) with different crops, and performed a statistical
test to determine if the \twod{} model can provide a significantly
better fit than the \oned{} model.

Both sites were using ridge systems and had \threed{}
datasets. However, since the root density model was intended to be
used in conjunction with Daisy, and the soil module of Daisy is
\twod{} and assumes a flat soil surface, the root density model had to
have the same properties. The geometry mapping represents an
additional uncertainty, especially in conjunction with the estimated
physical quantities, $d_c$, $w_c$, and $M_r$.
...
the number of data points. For the CAU datasets, a comparable narrow
root zone is estimated by the \twod{} model fit, and as a result, we
generally get a better fit for the \twod{} model compared to the
\oned{} model.
...
and~\ref{fig:cau2008}) confirms that the model provides a good
...
expect the \twod{} model in general to perform better in the early
...
the below ground development rather than above ground production or
water and nitrogen balances, the model is inadequate.  In particular,
for solute and water uptake, a dynamic root density model would be
needed to explore how the development is affected by the local soil
conditions, such as mechanical resistance, aeration, or availability
