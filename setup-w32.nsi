...
Name "Daisy ${VERSION}"
OutFile "daisy-${VERSION}-setup-w32.exe"
InstallDir "$PROGRAMFILES\Daisy ${VERSION}"
InstallDirRegKey HKCU "Software\Daisy ${VERSION} (w32)" "Install Directory"
...
Section "Main Install"
  SetOutPath $INSTDIR
  File /r "install\*.*"
  WriteRegStr HKCU "Software\Daisy ${VERSION} (w32)" "Install Directory" $INSTDIR
...
Section "Uninstall"
  ...
  RMDir /r "$INSTDIR"
  DeleteRegValue HKCU "Software\Daisy ${VERSION} (w32)" "Install Directory"
  DeleteRegKey /ifempty HKCU "Software\Daisy ${VERSION} (w32)"
