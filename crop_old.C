...
public:
  ...
  void initialize (const Units&, const Geometry& geo, OrganicMatter&, double, 
                   const Time&, Treelog&);
...
void
CropOld::initialize (const Units& units,
                     const Geometry& geo, const double row_width,
                     const double, const double seed,
                     OrganicMatter& organic_matter, double SoilLimit,
...
void
CropOld::initialize (const Units& units,
                     const Geometry& geo, OrganicMatter&, double, 
                     const Time&, Treelog&)
{
...
static struct CropOldSyntax : public DeclareModel
{
  Model* make (Block& al) const
  { return new CropOld (al); }
  CropOldSyntax ()
    : DeclareModel (Crop::component, "old", "\
Daisy v1 crop model, for crops with storage organ in the shoot.")
  ...
  void load_frame (Frame& frame) const
  {
    ...
    Canopy.add ("HvsDS", Value::None (), "cm", Value::Const,
...
Non-functional lim for N-concentration in roots.");
    ...
    // HarvestPar
    Harvest.add ("index", Value::None (), Value::Const,
                 "Fraction of leaf that are storage organ.");
    Harvest.add ("beta", Value::None (), Value::Const,
                 "The root/top concentration relation.");
    Harvest.add ("CStem", "g N/g DM", Value::Const,
                 "Normal straw concentration.");
    Harvest.add ("CSOrg", "g N/g DM", Value::Const,
                 "Sorg concentration at the end of the normal range.");
    Harvest.add ("alpha", Value::None (), Value::Const,
                 "Relative increase in straw concentration above normal range.");
    Harvest.add_submodule_sequence ("Stem", Value::Const, 
    ...
    Harvest.add ("C_Stem", Value::None (), Value::Const,
...
Maximal development stage for which the crop survives harvest.");
    ...
    CrpAux.add ("LogPotCanopyAss", "g CH2O/m^2", Value::LogOnly,
                "Yesterdays total potential canopy assimilation.");
    CrpAux.add ("LogCanopyAss", "g CH2O/m^2", Value::LogOnly,
                "Yesterdays total canopy assimilation.");
    CrpAux.add ("IncWLeaf", "g DM/m^2/d", Value::LogOnly, "Leaf growth.");
    CrpAux.add ("IncWRoot", "g DM/m^2/d", Value::LogOnly, "Root growth.");
    CrpAux.add ("H2OUpt", "mm/h", Value::State, "H2O uptake.");
    vCrpAux.add ("H2OUpt", 0.0);
    CrpAux.add ("NH4Upt", "g N/m^2/h", Value::LogOnly, "NH4-N uptake.");
    CrpAux.add ("NO3Upt", "g N/m^2/h", Value::LogOnly, "NO3-N uptake.");
    CrpAux.add ("Fixated", "g N/m^2/h", Value::LogOnly, " fixation from air.");
    ...
    frame.add ("enable_water_stress", Value::Boolean, Value::Const,
                "True, if water stress can limit crop growth.");
    frame.add ("enable_water_stress", true);
    frame.add ("enable_N_stress", Value::Boolean, Value::Const,
                "True, if nitrogen stress can limit crop growth.");
    frame.add ("enable_N_stress", true);
...
void 
CropOld::Emergence (const Geometry& geo, const SoilHeat& soil_heat)
{
...
void 
CropOld::RootPenetration (const Geometry& geo,
...
void 
CropOld::RootDensity (const Geometry& geo)
{
  const Parameters::RootPar& Root = par.Root;
                        ...
                        const SoilHeat& soil_heat)
{
  ...
  const double T_soil
    = geo.content_height (soil_heat, &SoilHeat::T, -Depth/3.0);
  const double RMRoot
...
double
CropOld::SOrg_DM () const
{ 
  return var.Prod.WLeaf * par.Harvest.index
    * 10;                       // [g/m^2 -> kg/ha]
}
