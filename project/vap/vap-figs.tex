...
Using the local estimated parameter values for dimethoate at Estrup
does not visibly affects the simulation results
(figure~\ref{fig:Estrup-acc}).  However, using the lower measured
sorption rate (especially in the subsoil) for metamitron at Silstrup
does result in larger simulated drain leaching during the measurement
period, as shown on figure~\ref{fig:local-metamitron}.  The values
used in Daisy for fenpropimorph are within the span measured at the
other sites, except that degradation apparently descrease faster with
depth than asserted by \focus{}.

\begin{figure}[htbp]
  \begin{center}
    \hspace*{-2cm}\includegraphics{local/fig/Silstrup-Metamitron-acc.pdf}
  \end{center}
  \caption{Comparison of simulation of Metamitron at the Siltrup site
    using sorption parameter from \citet{ppdb20100517} with a
    simulation with local estimated sorption parameters
    \citep{vap2003}.}
  \label{fig:local-metamitron}
\end{figure}
