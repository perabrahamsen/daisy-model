...
chose to end the simulation 2002-03-11, rather than continue with
...
\begin{figure}[htbp]
  ...
  \caption{Accumulated precipitation and hourly values for temperature
    ...
    potential and simulated actual evapotranspiration are also shown.}
  \label{fig:weather}
\end{figure}
...
pesticides, until it become incorporated into the soil by either
tillage operations or earthworm activity.  However, due to the timing
of the applications, both effect is likely to be negligible.

\subsection{Crop management}
\label{sec:crop-man}

For sowing information about date, crop and sowing density were
provided.  The Daisy crop parametrization do not rely on sowing
...
earth worm channels.  Apart from the soil water measurements, both
...
The highest glyphosate concentrations in both sites were seen right after
...
In \citet{madsen2000pesticide} sorption parameters are measured for a
...
conductivity for the plow layer in both sites based on the measurements
...
so large that the capillary forces are no longer dominating.  On Daisy
...
in that interval, and instead specify an anisotropy of 100.  This means
...
wrong, but we don't expect large horizontal gradients in that case
anyway.
...
original \macro{} parametrization and the modified parametrization
...
at which point the biopores will deactivate.  The capability of the
biopores to extract water is further limited by the storage capacity
of the biopores themselves, and the ability to pass the water back to
...
Drain depth and distance are taken from \citet{lindhardt2001}, and can
be used directly by Daisy.  Automatic measurements of groundwater
...
Silstrup corresponds to ponding, where the top of the soil will become
saturated.
\begin{figure}[htbp]
  \begin{center}
    \figtop{Silstrup-gw}
...
However, since bioincorporation of litter into the soil is part of that
module, we had to re-enable it as the litter layer appeared to be significant.
...
We now got water, but not significant amounts of glyphosate, in the
...
Furthermore, \citet{gjettermann2009} demonstrated that glyphosate did not
...
highest intensity 1.4 mm/h).  It is therefore likely that the hydraulic
conductivity is still high at that point.  We chose to add crust
2000-06-01 (after 48.4 mm rain, max intensity 4.9 mm/h), setting the
...
than the local detention capacity in part of the field, the surplus
...
that a local detention capacity of 2 mm would result 10\% of the total
