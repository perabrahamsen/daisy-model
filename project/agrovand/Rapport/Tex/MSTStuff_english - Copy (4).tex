...
\begin{equation}
\frac{\partial \theta_{\text{1}}}{\partial t}=-\nabla \cdot \mathbf{q}_{\text{1}}
-\Gamma_{\text{w1}} \label{eq:continuity}
\end{equation}
where $\theta_{\text{1}}$ is the volumetric water content and
$\Gamma_{w1}$ is the sink term for water. The partial differential
equation can be developed by combining Darcy´s law, equation
(\ref{eq:darcy}) and the mass balance, equation
(\ref{eq:continuity}), thus
\begin{equation}
\frac{\partial \theta_{\text{1}}}{\partial t}=\nabla \cdot
\left(\mathbf{K}(\psi)\nabla (\psi + z)\right) - \Gamma_{\text{w1}}
\label{eq:richards}
\end{equation}
...
\begin{cases}
\frac{\partial \theta_{\text{1}}}{\partial t}=\nabla \cdot
\left(\mathbf{K}(\psi)\nabla (\psi + z) \right)-\Gamma_{\text{w1}} & \text{in}\  \Omega \\
\mathbf{\bar{n}} \cdot \left(\mathbf{K}(\psi)\nabla (\psi + z)
\right)=
-q_{\text{1}} & \text{on}\ \partial \Omega^{N} \\
...
$q_{\text{1}}$ is the magnitude of the outward flow from the
...
\begin{equation}
q_{\text{1}}=\mathbf{\bar{n}} \cdot \begin{bmatrix}0 \\ K_{zz}
\end{bmatrix}
\end{equation}
...
obtain
\begin{equation}
\int_{Q_i} \frac{\partial \theta_{\text{1}}}{\partial t} d \Omega =
\int_{\partial Q_i} \left(\mathbf{K}(\psi)\nabla (\psi + z)
\right)\cdot \mathbf{\bar{n}} dl - \int_{Q_i} \Gamma_{\text{w1}} d\Omega
\label{eq:integratet}
\end{equation}
%
where $\mathbf{\bar{n}}$ is the outwarded unit normal and
$\partial Q_i$ the boundary of $Q_i$. The cell averages of
$\theta_{\text{1}}$ and $\psi$ are denoted  $\theta_i$ and
...
\begin{equation}
|Q_{i}|\left(\frac{d \theta_{\text{1}}}{dt} \right)_i = \sum_{j \in \sigma_i} D_{ij}(\boldsymbol{\psi})
...
%
\begin{equation}
B_{ij'}^{N}(\boldsymbol{\psi})= -q_{\text{1,}ij'}|e_{ij'}|
\end{equation}
%
where $q_{\text{1,}ij'}$ is the size of the Darcy flux, perpendicular
to the cell face and positive for flux out from cell $i$. The easiest
way to implement Dirichlet boundary conditions is simply to force
$\psi_i$ to the value that $\psi$ has on the face with Dirichlet
conditions. Conflicts can arise if cell $i$ has more than one face with
a Dirichlet condition. Instead, the Dirichlet boundary condition is
implemented as if the midpoint of the Dirichlet face was a neighbor
cell. Similar to an interior cell face, a diffusive and a gravitational
...
%
\begin{equation}
\Gamma_{\text{w1}}=\Gamma_{\text{w1a}}+\Gamma_{\text{w1p}} \delta(x_p-x)\delta(z_p-z)
\end{equation}
%
where $\Gamma_{w1a}$ is the contribution from a area
distributed sink and $\Gamma_{w1a}$ is the contribution from a
point sink. $(x_p,z_p)$ are the coordinates of the point sink which
shall be placed in the interior of a cell(not on the cell faces). $\delta$
is the Dirac delta function. Thus, the contribution from the sink
terms to a cell yields
%
\begin{equation}
S_{i}(\boldsymbol{\psi}) = \Gamma_{\text{w1a}} |Q_i| + \Gamma_{\text{w1p}}
\end{equation}
%
...
(point sources). Both $\Gamma_{w1a}$ and $\Gamma_{w1p}$
can be dependent on the solution ($\psi$).
...
arithmetic, logarithmic of harmonic mean. Physical arguments speak
for applying the harmonic mean:
\begin{equation}
...
right. Each iteration starts with the most left cell, then the second
from the left, then the third and so on. For a given cell, the water
level is compared to the cell to the right and if the water level in the
left cell is highest, the water level in the two cells is equaled and the
water is conserved. The only violation to this rule is that the water
in the detention storage not is moved. The iteration procedure ends
when the exchanges of water between cells are below a certain
(very low) level. In figure \ref{fig:surface_negative} is the effect of
the \textit{Negative} surface model shown.
...
flow. The model corresponds to the \textit{0D} model with
infinitelyAdvectio large detention storage. In figure
\ref{fig:surface_none} is the effect of the \textit{None} surface
model shown.
...
In the \textit{Compare} model, the differences between the
surface water level in neighboring cells are calculated, but only for
...
then equaled (with water conservation), but still under the rule that
no water leaves the detention storage. In figure
...
In order to get rid of $\theta_{\text{1}}$ at iteration step
...
\theta_{\text{1}}^{n+1,m+1}&=\theta_{\text{1}}^{n+1,m}
+\frac{d\theta_{\text{1}}}{d\psi}\mid^{n+1,m}(\psi^{n+1,m+1}-\psi^{n+1,m})\\
&=\theta_{\text{1}}^{n+1,m} +C^{n+1,m}(\psi^{n+1,m+1}-\psi^{n+1,m})
\label{eq:taylor}
\end{split}\end{equation}
%
where $C=\partial \theta_{\text{1}}/\partial \psi$ is the specific
water capacity function. The time derivative of
$\theta_{\text{1}}$ can then be approximated as:
%
\begin{equation}\begin{split}
\frac{\partial \theta_{\text{1}}}{\partial t}&\approx
\frac{\theta_{\text{1}}^{n+1,m+1}-\theta_{\text{1}}^{n}}{\Delta
  t}=\frac{\theta_{\text{1}}^{n+1,m+1}-\theta_{\text{1}}^{n+1,m}}{\Delta
  t}+\frac{\theta_{\text{1}}^{n+1,m}-\theta_{\text{1}}^{n}}{\Delta t}\\ & \approx C^{n+1,m}
\frac{\psi^{n+1,m+1}-\psi^{n+1,m}}{\Delta
  t}+\frac{\theta_{\text{1}}^{n+1,m}-\theta_{\text{1}}^{n}}{\Delta t}
\end{split}\end{equation}
%
...
%%% with moustages %%%
\begin{equation}
\theta_{\text{1}}=\begin{cases} \theta_{r} +
...
\begin{equation}
S_e=\frac{\theta_{\text{1}}-\theta_r}{\theta_s-\theta_r}
\end{equation}
%
...
water content, $\theta_{\text{1}}$. From the expression for the
...
\section{Mobile-immobile transport}\index{mobile-immobile_transport}

For describing the solute movement, the soil matrix water is divided
into two parts, a mobile and an immobile part
%
\begin{equation}
\theta_{\text{1}}= \theta_{\text{m}} + \theta_{\text{im}}
\end{equation}
%
where $\theta_{\text{m}}$ and $\theta_{\text{im}}$ are the
mobile and immobile water content, respectively. The immobile part
of $\theta_{\text{1}}$ is always filled first. Reversely is the mobile
part emptied first. Thus, the immobile part of $\theta_{\text{1}}$
can be expressed as
%
\begin{equation}
\theta_{\text{im}}=
\begin{cases}
\theta & \text{for\ } \theta_{1} < \theta_{\text{im,max}} \\
\theta_{\text{im,max}} & \text{for\ } \theta \geq \theta_{\text{im,max}}
\end{cases}
\end{equation}
%
...
\theta_{\text{m}}=
\begin{cases}
 0 & \text{for\ } \theta < \theta_{\text{im,max}} \\
\theta -\theta_{\text{im,max}} & \text{for\ } \theta \geq \theta_{\text{im,max}}
\end{cases}
\end{equation}
%
The solute concentration is similarly divided into a part associated
with the immobile water, $C_{\text{im}}$ and a part associated
with the mobile water, $C_{\text{m}}$. Dividing the matrix water
content into a mobile and an immobile volume is somewhat
inconsistent when $\theta_{\text{im,max}}>\theta_r$ where
$\theta_r$ is the residual water content. When comparing to
Richards' equation all water above $\theta_r$ is subject to
movement. From Richards' equation the Darcy velocity can be
calculated as $\mathbf{v}=\mathbf{q}/\theta$. The flux,
$\mathbf{q}$ as used for calculating the movement of solute in
the mobile volume is calculated as
%
\begin{equation}
\mathbf{q}_{\text{m}} =
\begin{cases}
\mathbf{0} & \text{for\ } \theta_{\text{m}} = 0 \\
\mathbf{q} & \text{for\ } \theta_{\text{m}} > 0
\end{cases}
\label{eq:mobileflux}
\end{equation}
%
...
%
\begin{equation}
\mathbf{v}_{\text{m}}=\mathbf{q}_{\text{m}}/\theta_{\text{m}}
\label{eq:mobilevelocity}
\end{equation}
%
It should be remarked that the velocity $\mathbf{v}_{\text{m}}$
can be very large for a small mobile water content,
$\theta_{\text{m}}$.
%
The exchange of solutes between the mobile and the immobile zone
are driven by the concentration differences. The sink to the mobile,
water, $\Gamma_s$ yields:
%
\begin{equation}
\Gamma_s = \frac{\partial (\theta_{\text{im}}C_{\text{im}})}{\partial t}
= \alpha_s (C_{\text{m}}-C_{\text{im}})
\label{eq:sink_mob_immob}
\end{equation}
...
the matrix part (non macroporous part) of the soil:
...
%
\begin{equation}
\mathbf{j}=\mathbf{q}_{\text{m}}C_{\text{m}}
\end{equation}
%
...
dispersion tensor is based on $\mathbf{v}$ and not
$\mathbf{v}_{\text{m}}$. The last can result in infinitely
large values for $\theta=\theta_{\text{m}}$.\\
\\
...
\end{equation}
%
The value is based on the total water content $\theta$ and not only
the mobile water content, $\theta_{\text{m}}$. If we are using
equation (\ref{eq:Millington}) and expressing the mean velocity in
the pores associated with solute movement by
$\mathbf{q}_{\text{m}}$ and $\theta_{\text{m}}$, the
elements of $\theta_{\text{m}}\mathbf{D}$ can be expressed
as:
%
\begin{equation}
\begin{split}
&\theta_{\text{m}}
D_{xx}=\alpha_L\frac{\theta_{\text{m}}}{\theta}\frac{q_{x}^{2}}{|\mathbf{q}|}
+\alpha_T\frac{\theta_{\text{m}}}{\theta}\frac{q_{z}^{2}}
{|\mathbf{q}|}+D_0\theta_{\text{m}}\frac{\theta^{7/3}}{\theta_s} \\
&\theta_{\text{m}}
D_{zz}=\alpha_L\frac{\theta_{\text{m}}}{\theta}\frac{q_{z}^{2}}{|\mathbf{q}|}
+\alpha_T\frac{\theta_{\text{m}}}{\theta}\frac{q_{x}^{2}}
{|\mathbf{q}|}+D_0\theta _{\text{m}}\frac{\theta^{7/3}}{\theta_s} \\
&\theta_{\text{m}} D_{xz}=\theta_{\text{m}}
  D_{zx}=(\alpha_L-\alpha_T)\frac{\theta_{\text{m}}}{\theta}\frac{q_{x}q_{z}}{|\mathbf{q}|}
\end{split}
\label{eq:thetad}
\end{equation}
%
...
%
\begin{equation}
\mathbf{j}_{\text{m}}=\theta_{\text{m}} C_{\text{m}}\mathbf{v}_{\text{m}}
-\theta_{\text{m}}\mathbf{D}\nabla C_{\text{m}}=C_{\text{m}}
\mathbf{q}_{\text{m}}-\theta_{\text{m}}\mathbf{D}\nabla C_{\text{m}}
...
(\theta_{\text{m}}
  C_{\text{m}})}{\partial t} + \frac{\partial (\theta_{\text{im}}
  C_{\text{im}})}{\partial t} + \frac{\partial (\theta_{\text{mp}}
  C_{\text{mp}})}{\partial t} \nonumber \\ =-\nabla \cdot \mathbf{j}_{\text{m}}-\nabla
  \cdot \mathbf{j}_{\text{mp}} - \Gamma
...
$\Gamma$ is the sink term of the solute. \\
\\
...
%
\begin{equation}
\frac{\partial (\theta_{\text{m}} C_{\text{m}})} {\partial t}
   =-\nabla \cdot \mathbf{j}_{\text{m}} - \Gamma_{\text{m}}
\label{eq:solmassbal_im}
\end{equation}
%
where $\Gamma_{\text{m}}$ is the sink term which remove
solutes from the mobile matrix water. The removed (or added)
solute can be absorbed, moved to the immobile water
($\Gamma_s$ as expressed by equation
(\ref{eq:sink_mob_immob})) or the macropore domain or be
subject to chemical or biological reduction.

The boundary conditions to the equation specifies a combination of
$C_{\text{m}}$ and its derivative on the boundary. Also here, the
...
\begin{equation}
C_{\text{m}}=C_{\text{m,0}} \label{eq:Dirichlet_sol}
\end{equation}
%
where $C_{\text{m,0}}$ is the predescribed concentration. The
Neumann boundary condition is:
%
\begin{equation}
 \mathbf{\bar{n}}\cdot (C_{\text{m}}\mathbf{q}_{\text{m}}-\theta_{\text{m}}\mathbf{D}\nabla
 C_{\text{m}})=\mathbf{\bar{n}}\cdot \mathbf{j}_{\text{m}}=j_{\text{m}}
\label{eq:Neumann_sol}
\end{equation}
%
where $j_{\text{m}}$ is the size of the flux, positive for outward
flux. As boundary condition for the ingoing flow
$j_{\text{m}}=\mathbf{\bar{n}}\cdot\mathbf{q}_{\text{m}}C_{\text{m,0}}=q_{\text{m}}C_{\text{m,0}}$
is often used where $C_{\text{m,0}}$ is the concentration of the
flow. As lower boundary condition is
$j_{\text{m}}=\mathbf{\bar{n}}\cdot\mathbf{q}_{\text{m}}C_{\text{m}}=q_{\text{m}}C_{\text{m}}$
...
\begin{equation}
\begin{cases}
 \theta_{\text{m}} \frac{\partial C_{\text{m}}}{\partial t} + C_{\text{m}} \frac{\partial
  \theta_{\text{m}}}{\partial t} =  -\nabla \cdot(C_{\text{m}}
  \mathbf{q}_{\text{m}}-\theta_{\text{m}}
  \mathbf{D}\nabla C_{\text{m}})- \Gamma_{\text{m}} &
  \text{in} \ \Omega \\
 \mathbf{\bar{n}}\cdot (C_{\text{m}}\mathbf{q}_{\text{m}}-\theta_{\text{m}}\mathbf{D}\nabla C_{\text{m}})=j_{\text{m}} &
  \text{on}\  \partial\Omega_1 \\
 C_{\text{m}}=C_{\text{m,0}} & \text{on}\ \partial\Omega_2
\end{cases}
...
%
\begin{equation}
\text{P}_{\text{e}}=v_{\text{m}}\Delta x/D
\label{eq:Peclet}
\end{equation}
%
...
%
\begin{equation}
\text{C}_{\text{r}}=v_{\text{m}}\Delta t/\Delta x
\label{eq:Courant}
\end{equation}
%
...
that the Peclet number raises over 2?. For simulations where a part
of the water not is divided an immobile region, i.e
$\theta_{\text{im}}=0$ and $\theta_{\text{m}}=\theta$, the
Peclet number for the flow in the x-direction is:
%
\begin{equation}
\begin{split}
(\text{P}_{\text{e}})_x&=\frac{q_{\text{m},x} \Delta x}{\theta_{\text{m}} D_{xx}}
= \frac{q_x \Delta x}{\theta D_{xx}} = \frac{q_x \Delta x}{\alpha_L
...
$(\text{P}_{\text{e}})_y$. It can then be concluded that the
...
%
\begin{equation}
\bar{\alpha}_L=\begin{cases} \frac{|\mathbf{v}_{\text{m}}|\Delta
    t}{\gamma}-\alpha_L-\frac{D^{*}}{|\mathbf{v}_{\text{m}}|}, & \text{for} \ \alpha_L +
    \frac{D^{*}}{|\mathbf{v}_{\text{m}}|} < \frac{|\mathbf{v}_{\text{m}}|\Delta t}{\gamma}
 \\ 0, & \text{for} \ \alpha_L +
    \frac{D^{*}}{|\mathbf{v}_{\text{m}}|} \geq \frac{|\mathbf{v}_{\text{m}}|\Delta
    t}{\gamma}\end{cases}
\end{equation}
...
To investigate the stability of the numerical model is a simple system
 ...
 column with steady-state water flow with pore velocity $v$. There is no immobile
 water, $\theta_{\text{im}}$ thus $v_{\text{im}}=v$.
...
%
\begin{equation}
\frac{\partial C_{\text{m}}}{\partial t}=D\frac{\partial^2C_{\text{m}}}{\partial
  x^2}-v\frac{\partial C_{\text{m}}}{\partial x}
\end{equation}
%
...
%
\begin{equation}
\left(-D\frac{\partial C_{\text{m}}}{\partial x}+vC_{\text{m}} \right)\bigg\vert_{x=0}
=vC_{\text{m,0}}
\end{equation}
...
\begin{equation}
\begin{split}
C\text{m}(x,t) = &\frac{1}{2} C_{\text{m,0}}  \erfc\left
  [\frac{x-vt}{2(Dt)^{1/2}}\right]+\left(\frac{v^2t}{\pi
  D}\right)\exp\left[-\frac{(x-vt)^2}{4Dt}\right ] \\
&-  \frac{1}{2} C_{\text{m,0}} (1+\frac{vx}{D}+\frac{v^2t}{D})\exp(vx/D)\erfc\left[\frac{x+vt}{2(Dt)^{1/2}}\right]
\end{split}
\end{equation}
...
$C_{\text{m,0}}$ is for the simplicity chosen to 1 and $D=0.05$
...
%
\begin{equation}
\left(-\theta_{\text{m}} D_{zz} \frac{\partial C_{\text{m}}}{\partial z}
+q_zC_{\text{m}} \right) \bigg \vert_{z=z_{\text{surf}}}
...
steady state (i.e. $\partial \theta_{\text{m}} / \partial t=0$ and
constant $\mathbf{q}$). The immobile water content,
$\theta_{\text{im}}=0$ and all the adsorption processes are
going through the mobile water to the sorped phase. These
conditions are seldom fulfilled in the 'real life' where both the water
content and the flux are time-dependent. For testing the solute
transport model is a situations with steady state
...
$C\text{m}$. The bulk density is assumed to be constant through
...
&\rho_b\frac{\partial C_a}{\partial t}+\frac{\partial
  (C_{\text{m}}\theta_{\text{m}})}{\partial t}=\rho_b\frac{\partial C_a}{\partial
  C_{\text{m}}}\cdot \frac{\partial C_{\text{m}}}{\partial t}
+\theta_{\text{m}} \frac{\partial C_{\text{m}}}{\partial t}+ C_{\text{m}}\frac{\partial
  \theta_{\text{m}}}{\partial t} = \\
&\theta_{\text{m}} R \frac{\partial C_{\text{m}}}{\partial t} + C_{\text{m}} \frac{\partial
  \theta_{\text{m}}}{\partial t}
\end{split}
...
%
\begin{equation}
R=\frac{\rho_b}{\theta_{\text{m}}} \cdot \frac{\partial C_a}{\partial
C_{\text{m}}}+1 \label{eq:retardationfactor}
\end{equation}
%
The most simple adsorption isotherm is the linear adsorption where
$C_a=K_dC_{\text{m}}$ and as a consequence
$R=1+\frac{\rho_bK_d}{\theta_{\text{m}}}$. \\
\\
...
\begin{equation}
\begin{split}
 R\theta_{\text{m}} \frac{\partial C_{\text{m}}}{\partial t} +& C_{\text{m}} \frac{\partial
  \theta_{\text{m}}}{\partial t} =  -\nabla \cdot(C_{\text{m}}
  \mathbf{q}_{\text{m}}-\theta_{\text{m}} \mathbf{D}\nabla C_{\text{m}})- \theta_{\text{m}} \mu_l C_{\text{m}} +
  \rho_b \mu_s
\label{eq:advdisp}
\end{split}
\end{equation}
...
%
\begin{equation}
R\frac{\partial C_{\text{m}}}{\partial t}=D\frac{\partial^2C_{\text{m}}}{\partial
  x^2}-v\frac{\partial C_{\text{m}}}{\partial x}-\mu C_{\text{m}} +\gamma
\end{equation}
%
...
\begin{equation}
\begin{small}
C_{\text{m}}=\begin{cases}\frac{\gamma}{\mu}+ (C_{\text{m,i}}-\frac{\gamma}{\mu})A(x,t)+(C_{\text{m,0}}-\frac{\gamma}{\mu})B(x,t), 0<t\leq t_0 \\
\frac{\gamma}{\mu} + (C_{\text{m,i}}-\frac{\gamma}{\mu})A(x,t)+(C_{\text{m,0}}-\frac{\gamma}{\mu})B(x,t)-C_{\text{m,0}}B(x,t-t_0), t>t_0
\end{cases}
\end{small}
\end{equation}
%
...
are
calculated as in the existing 1D Daisy \\
\\
