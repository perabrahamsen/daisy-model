...
the four drain seasons between between 1998 and 2002, which included
...
TDR probes, and they only one we have used directly for our
calibration.  The piezometer measurements have been analyzed for use
calibration of the lower boundary, see appendix~\ref{app:piezometers}.
Furthermore, transport pathways have been explored using dye tracer,
and biopores have been counted both in the original project, and in
...
subcomponents of the the newly developed 2D Daisy against real data:
...
data suffer from the effect of ice, which is not simulated.
...
interested in the individual events.  For simplicity, we chose to full
...
Furthermore, Daisy use information about the seed bed preparation.  As
...
(\citet{kjaersgaard2008crop}).  The the default parametrization was
...
calibration was done on the pesticides.  See
...
cm) difference between treatments and between spring and autumn, we
...
\begin{table}[htbp]
  \caption{Soil properties. Depth is specified in cm below soil surface, 
    ...
    Humus is given as a percentage of total weight.  For the drain canyon, 
...
canyon would be directly connected to the drain pipes.  Based on
...
and in the C horizon is has been tripled.  The result is shown on
...
to match drain flow, and is shown on figure~\ref{fig:gwt}.  Note that
...
connected biopores.  The TDR and soil bromide measurements were all
performed in the part of the field with clay underground, and the
...
transport.  This division was exclusively calibrated based on bromide
...
primary domain where.  Otherwise, the water in the soil corresponding
...
pure convection-dispersion equation.  In these simulations, the bromide
...
primary and secondary domain, we use difference filter coefficients
for the two domains.  We choose values of 80 and 40 m$^-1$ for the
primary and secondary domain respectively, in order to stay near the
...
in figures~\ref{fig:first} to~\ref{fig:last}, found at the end of the
...
apparent noise in the measurements during periods with frost.
Enabling the experimental support for ice in Daisy showed a good match
between upper TDR probes and simulated water during the two winter
periods, supporting the idea that difference is due to ice.
Unfortunately, the ice support in Daisy is not yet complete, and
created too many other problems with the simulation, so it was
disabled for the final runs. The simulation overestimate the water
level near the soil surface, which could possibly be a problem with
the TDR measuring some air.  We may overestimate the dynamics near the
bottom of the plow layer.  The measurements for the bottom TDR show
fast variations during the winter which looks mostly like noise,
something not duplicated in the simulation, with or without ice.
...
underestimated.  The two next graphs below that divide the content in
the same intervals into the primary domain (small pores, slow water
movement) and the secondary domain (large pores, fast water movement).
The remaining graphs shows bromide transport through the borders
between the soil intervals.  As can be seen, the bromide enter the
soil through the secondary domain, and some move further down through
the secondary domain at 25 cm, but most bromide are moved down through
the tertiary domain (the biopores), There is no significant transport
in the primary domain.  However, at the end of the period the primary
...
flow proportional particle leaching, with simulated values extracted
the same time as measurements.  Each data point represents the
accumulated value since last measurement.  We also plot the simulated
reservoir of readily available particles from the
...
accumulated numbers shows that seen over an event the simulation is
...
is larger than the variation in the simulated numbers.  We can also
...
of bromide and pesticides are shown for the seasons they were
measured.  For bromide, we also show accumulated values.  For
pesticides, where there are far fewer measurements, we instead show
dynamic leaching.  

The bromide show too high concentration at the beginning of the
season, especially after the first large event that activates the
biopores.  The simulated bromide concentration is too low at the end
of the season.  The second season continue this trend, with too low
concentrations.
...
drain leaching is closer to what is measured.

\chapter{Discussion}

The simulation results can roughly be divided into three categories.
The first category is the results that depends mostly on the upper
part of the system.  These include TDR probes, drain particles, and
pesticide leaching through drains.  The second category is the results
that depends mostly on the lower part of the system.  These include
...
gives us faith in both our upper boundary, and our description of the
part of the soil monitored by the probes.  The only caveat here is ice
and snow, which were not included in our final simulation.

During calibration, we found that the amount of particles simulated in
drain pipes were robust with regard to changes to the lower boundary,
...
pipes is relatively robust with regard to changes in the lower
...
calibrated.  A (de)sorption rate of 0.05 h$^-1$ was chosen as a match
for the Daisy timestep.  Given these caveats, the results are
encouraging.
...
indicates that large part of the field has a sandy underground, and
are unlikely to contribute to the drain flow.  Finally, the
piezometers shows great spatial and temporal variation, and indicate
that different parts of the field may contribute to the drain flow and
different times.  As we sometimes have significant drain flow when the
piezometers show low pressure, could indicate that local areas of
...
Dividing the field in to two parts, one with free drainage and one
with an aquitard bottom, is not enough to catch this.  The main
problem is our inability to catch the dynamic effect in the drain that
occurs a few hours into a large event.  Figure~\ref{fig:gwt-sim} shows
the other side of this, our simulated groundwater table is much more
stable than the median piezometer measurements, despite the later
representing pressure 2.3 m below ground level.

\section{Bromide and pathways}

The soil measurements shows that largest amounts of bromide should be
located between 50 and 100 cm below soil surface at the end of the
...
The effect can clearly be seen on the top left graph of
...
and~\ref{fig:bromide-acc}.  The first event activate both biopore
classes, the later events mostly the deep biopores.  We haven't yet
determined why this is.

In general, this indicate a problem with our model of the pathways in
the system, which will likely affect not only bromide but also
...
development.  Ice obviously had a influence on the TDR measurements,
especially the first season.  This could be used for finishing the
dormant ice module of Daisy, and would increase the trustworthiness of
the simulation for that season, especially when coupled with the drain
measurements.  The effect of frost on particle generation, as examined
by e.g.~\citet{kvarno2006influence} would be relevant.  And of course,
...
developing a model that included the effect of tillage on particle
leaching, and consequently on leaching of strongly sorbing pesticides.
A better model including tillage more directly might help explain the
difference we measured between the three seasons.
...
graphs shows highest pressure closest to the well, and that at a
