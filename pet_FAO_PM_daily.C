// pet_FAO_PM_daily.C -- FAO ETc using Penman-Monteith with daily data
// 
...
class PetFAO_PM_daily : public Pet
{
  // Parameters.
...
public:
  ...
  PetFAO_PM_daily (const BlockModel& al)
    : Pet (al),
...
void
PetFAO_PM_daily::tick (const Time&, const Weather& weather, const double /* Rn */,
                        const Vegetation& crops,
...
static struct PetFAO_PM_dailySyntax : public DeclareModel
{
  Model* make (const BlockModel& al) const
  { return new PetFAO_PM_daily (al); }
  PetFAO_PM_dailySyntax ()
    : DeclareModel (Pet::component, "FAO_PM_daily",
                    "Potential evopotranspiration using Penman-Monteith.\n\
Based on daily weather data and a day cycle.")
  { }
...
} PetFAO_PM_daily_syntax;

// pet_FAO_PM_daily.C ends here.
