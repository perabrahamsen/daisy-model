...
public:
  ...
  // Create and Destroy.
public:
  WeatherSimple (const BlockModel&);
  ~WeatherSimple ();
};
...
WeatherSimple::WeatherSimple (const BlockModel& al)
  : WeatherOld (al),
...
A weather model with precipitation at regular intervals.")
  ...
  void load_frame (Frame& frame) const
    { 
      frame.declare ("precipitation_value", "mm/h", Attribute::Const,
		  "Amount of precipitation.");
      frame.set ("precipitation_value", 0.0);
      frame.declare_integer ("interval", Attribute::Const,
