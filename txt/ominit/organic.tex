...
modification is documented in \cite{mueller-smb}, and was an
...
\cite{daisy-somnew}. This was a complete recalibration that took into
...
model in \daisy{} is given in \cite{daisy-def}.
...
property of the system being modeled. The first step of a new
initialization procedure was conducted by \cite{daisy-init}. They
found that the ratio between the \textsc{som1} and \textsc{som2} for a
system in equilibrium would be 49\% vs 51\%. This recalibration
required the users to readjust the partitioning between \textsc{som1}
and \textsc{som2}. However, there was a strong tendency that the
"non-experts" users just picked the equilibrium values.
