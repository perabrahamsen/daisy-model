...
\subsubsection{Knowns and unknowns}
...
\begin{equation}
  \label{eq:smb-eq}
  \Delta\mbox{\textsc{smb}}i = 0 ;i = 1\ldots\mbox{\#\textsc{smb}}
\end{equation}
This is reasonable as the \textsc{smb} pools tend to asjust to input
...
the system without incrasing the number of unknowns.
\begin{equation}
  \label{eq:total}
...
\begin{equation}
  \label{eq:input}
  \Delta\mbox{\textsc{aom}}i = k_i ;i = 1\ldots\mbox{\#\textsc{aom}}
\end{equation}
...
\subsubsection{Explicit SOM partisioning}

The orginal, and still supported, method of initializing the
...
the system is overspecified, and we leave out one of the equations.
...
no longer applicable \textsc{som} partitioning hueristics developed
for the old model.

\subsubsection{Background mineralization}
...
initializating the system has the advantages of being model
...
Equation~\ref{eq:total}, so the total amound of (active) soil organic
...
The last and most common euilibrium initialization is to let the user
...
all the fast poools will quickly adapt to the input and to the size of
