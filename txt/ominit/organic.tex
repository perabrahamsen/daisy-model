...
\end{tabular}
 \caption{Long term simulation on a sandy soil (left column) and sandy
   loam soil (right column) of organic matter in percent of soil dry
   bulk mass (fully drawn line, left axes) and \textsc{som1} in
   percent of of total \textsc{som} (dotted line, right axes) with
   management changing from: (top row) high to low C input and (bottom
   row) low to high C input.  The $+$'s indicate the initialization of
   \textsc{som1} for the current humus level estimated by the
   quasi-equilibrium.}
  \label{fig:long}
\end{figure*}
...
\begin{figure}[htbp]
  ...
  \caption{Long term simulation on a sandy soil of organic matter in
