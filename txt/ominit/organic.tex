...
The generalized form for the dynamic change of the individual pools is
...
\begin{equation}
  ...
  \begin{array}{lcll}
    ...
    & & + \sum_{j=1}^{\mbox{\tiny\#\textsc{som}}} c_{i,j}\,\mbox{\textsc{aom}}j
    ...
    & & + \sum_{j=1}^{\mbox{\tiny\#\textsc{som}}} f_{i,j}\,\mbox{\textsc{aom}}j
...
For initialization purposes, we assume constant temperature and soil
humidity, which means $a_{i,j}$, $b_{i,j}$, $c_{i,j}$, $d_{i,j}$,
$e_{i,j}$, $f_{i,j}$ and $g_i$ will all be constants.  This leaves us
with one equation, and two unknowns (content and change) for each pool
in our system.  In order to find a solution, we will need to either
