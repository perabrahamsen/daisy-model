...
\begin{abstract}
  ...
  on densely populated homogenious fields to row crops that are dense
  in the rows but with a distance between rows, and demonstrate in
  each case how to find the distribution parameters from root dry
  matter and the size of the root zone.
\end{abstract}
...
$L_{0,0}$ is the the root density in the top of the row.  We then
define the following root dstribution
\begin{equation}
...
\begin{figure}[htbp]
  ZIG ZAG
  \caption{Overlapping root systems in half a row.}
  \label{fig:zigzag}
\end{figure}
...
\begin{equation}
  L^*_{z,x} =
    \begin{cases}
       \Sigma_{i=0}^{\infty} (L_{z,x + i R} + L_{z,R + i R - x}) & \text{if $x < R/2$}\\
                                                    0  & \text{if $x \geq R/2$}
    \end{cases}
  \label{eq:Lzxstar}
\end{equation}

Using (\ref{eq:Lzx}) and the rules for geometric serieses we can rewrite
the first case to get rid of the sum
\begin{equation}
  \begin{array}{rl}
     & \Sigma_{i=0}^{\infty} (L_{z,x + i R} + L_{z,R + i R - x})\\
    =& L_{0,0}\, e^{-a_z z} 
       \Sigma_{i=0}^{\infty} (e^{-a_x (x + i R)} + e^{-a_x (R + i R - x)})\\
    =& L_{0,0}\, e^{-a_z z} (       e^{-a_x x} \Sigma_{i=0}^{\infty} e^{-a_x i R} 
                          + e^{-a_x (R - x)} \Sigma_{i=0}^{\infty} e^{-a_x i R})\\
    =& L_{0,0}\, e^{-a_z z} (e^{-a_x x} + e^{-a_x (R - x)})
       \Sigma_{i=0}^{\infty} e^{-a_x i R}) \\
    =& L_{0,0}\, e^{-a_z z} (e^{-a_x x} + e^{-a_x (R - x)})
       \Sigma_{i=0}^{\infty} ((\frac{1}{e})^{a_x R})^i\\
