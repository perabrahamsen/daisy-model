...
(\ref{eq:root-integral}) in (\ref{eq:root-depth}) we get
\begin{equation}
  L_m = l_r \, a \, e^{-a d}
  \label{eq:a-only}
\end{equation}

If we substitute $W = -a d$ and isolate the known values on the right
side this gives us
\begin{equation}
...
\begin{eqnarray}
   a   &=& -\frac{W (-\frac{L_m d}{l_r})}{d}\\\label{eq:a-solved}
   L_0 &=& \frac{L_m}{e^{-a d}} = L_m e^{a d}\label{eq:L0-found}
\end{eqnarray}
...
$-L_m \frac{d}{l_r}$ is exactly at the bottom point ($-1 e^{-1}$), two
...
\item The solution for $W < -1$ represents a large $a$ parameter. From
  (\ref{eq:L0-found}) we see this also means $L_0$ is large.  Thus,
...
the root radius $r$ is known.  We define the root zone depth $d$ to be
the depth right below the row ($x = 0$) where the root density is
$L_m$.  As $x=0$ is the place where (\ref{eq:Lzx}) predicts the
highest density, the average root density at that depth will be well
below $L_m$.  Similarily, we define the radius $r$ as the horizontal
distance $r$ from the row where the root density at the surface ($z =
0$)
\begin{equation}
  L_m = L_{d,0} = L_{0,r}
  \label{eq:minroot}
