...
a static, empircal model. This model was validated indirectly, through
...
directly measured root density.  The proposed model is static in that it
doesn't include root growth, but it can be parametrized dynamically
...
wastewater for irrigation \citep{Forslund2010440,Surdyk2010451}, and
...
calculates root dry mass and root zone depth, and assumes a vertical root
...
The new \twod{} root density model implemented in Daisy and used in
the SAFIR project has not been presented in any of the previous
papers, nor has the root density measurement from the two Chinese
sites been published before. Only the static root distribution model
will be described and tested in this paper, the predictive crop growth
model in Daisy has been described elsewhere, and is outside the scope
of this paper.
...
\begin{figure}[htbp]
  ...
  \end{center}
  \caption{The root density model in the context of Daisy as applied
    in the SAFIR project. The crop growth module delivers the total
    root mass($M_r$), root zone depth ($d_c$), and root zone width
    ($w_c$) to the root density module, which delivers a spatial
    distribution of root density ($L$) to the uptake module. The
    uptake module is driven by potential transpiration and N demand,
    which come from the Soil-Vegetation-Atmosphere transfer (SVAT)
    module and the crop growth module, respectively.  Using the root
    density, the actual uptake is then calculated the hydraulic
    conductivity ($K$), the pressure head ($h$), and the nitrogen
    solute concentration ($C_N$) of nitrate and ammonium.  The later
    three are delivered by the soil module, which recieve sink terms
    for water and nitrogen ($S_\Theta$ and $S_N$) in return. The crop
    growth is mainly driven by assimilated CO$_2$, which is calculated
    by the SVAT module, based on weather data, actual transpiration,
    and the leaf area (LAI) and nitrogen status of leafs (RuBisCO-N).}
  \label{fig:daisy}
\end{figure}
