...
water in the soil, due to root signals. It has been shown that the
concentration of xylem ABA from the roots of plants can be described
as a function of water potential of the dry and wet zones
\citep{liu2008measurement,dodd2008accounting}. Several root models
...
with depth to match the observations.  These root models 
...
the second was water saving irrigation techniques, in particular PRD
...
density at that depth will be less than $L_m$.  Similarly, we define the
...
is physically meaningful.  It can be found using Newton's method with
an initial guess of $-3$. Knowing $Q$ we can find $a_z$~\MyEq{eq:Qaz},
$a_x$~\MyEq{eq:aztoax}, and $L_{0,0}$~\MyEq{eq:root-integral2}.
...
treatments were designed with the goal that the constituents of the
waste water should not affect crop growth.  Hence, for crop growth the
different irrigation sources could be seen as replicates.  The second
aspect was how the design of the irrigation system could best utilize
the water.  Two irrigation methods and three irrigation strategies
were considered.  The irrigation methods were subsurface drip
irrigation and furrow irrigation.  The irrigation strategies were full
irrigation (equivalent to potential evapotranspiration), deficit
...
At the CAAS site, root density was successfully sampled in 2008.  All
treatments were done with three replicates, but due to the labour
intensive nature, root density sampling was only performed on a subset
of the plots as outlined in Table~\ref{tab:caas2008treatments}. At
CAU, root density was successfully sampled in 2007 and 2008 at
selected plots, see Table~\ref{tab:cautreatments}.
...
15 cm).  At the z-axis, there were sampling in 10 cm intervals
...
illustrated in Fig.~\ref{fig:sample-caas}.  At the CAU site, the
...
flowering), and 2008-07-24 (second harvest). Both places roots were
...
organic debris were poured into a 250m mesh-sizes sieve for further
...
samples was determined according to Tennant (1975) with the
modification described by Ahmadi et al. (2011) and Andersen et
al. (1992) to keep the coefficient of variation around 10\% by
counting at least 100 intersects for each sample
...
the measurements with $z <$ 0 cm (above the flot soil surface of the
...
\twod{} model.  This means that we can use an partial F-test
