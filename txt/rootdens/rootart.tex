...
\author{Per Abrahamsen \and Mathias Neumann Andersen \and Søren Hansen
  \and Guitong Li \and Ping Li \and Mikkel Mollerup \and Xuebin Qi 
  \and Dongmei Qiao}
...
\institute{Per Abrahamsen \and Søren Hansen 
  \at Department of Basic Sciences and Environment,\\
      ...
      \email{pa@life.ku.dk}
  \and Mathias Neumann Andersen
  \at Department of Agroecology,\\
      University of Aarhus,\\
      Blichers Allé 20, DK-8830, Tjele, Denmark
  \and Mikkel Mollerup
  \at Road Directorate, Ministry of Transport, Denmark
  \and Ping Li \and Xuebin Qi \and Dongmei Qiao
  \at Water Resources and Water Environment\\
      Farmland Irrigation Research Institute\\
      Chinese Academy of Agricultural Science 
      and the Ministry of Water Resources \\
      380 East Hongli Road,\\
      Xinxiang City,Henan Province,453003, P.R.China
  \and Guitong Li
  \at Department of Soil and Water Science,\\
      College of Resources and Environment,\\
      China Agricultural University,\\
      No.2 of West road of Yuanmingyuan, Haidian, \\
...
\begin{abstract}
  \emph{Background and Aims} Water saving techniques such as
  alternating partial root zone techniques can be evaluated by
  mechanistic modelling of soil water and plant growth. This requires
  knowledge of the root density distribution, which is expensive to
  measure. A simple and robust model of the root density distribution
  is therefore desirable.
  
  \emph{Methods} An empirical model of vertical root density
  distribution was extended to two dimensions, and the parameters were
  derived in terms of root mass and root zone width and depth. Root
  density for potatoes (\textit{Solanum tuberosum} L.) and tomatoes
  (\textit{Solanum lycopersicum} L.) were measured at two sites.  Both
  the original and the extended model were fitted to these datasets. An
  F-test was used to determine whether the extended model was
  significantly better than the original model.

  \emph{Results} The extended model was superior when the root zone
  width was less than the distance between rows.  The median of the
  coefficient of determination for all the examined datasets was 0.86,
  and a visual inspection confirms that the model catches both the
  horizontal and vertical root density variation.

  \emph{Conclusions} We were able to adequately describe the measured
  data using a simple root density model, relying on three quantities,
  all physically meaningful.
\end{abstract}

\keywords{Root length density \and Root modeling \and Row crops 
  \and \textit{Solanum tuberosum} \and \textit{Solanum lycopersicum} 
  \and Partial root zone drying}

\section{Introduction}

With alternate side partial root zone drying (PRD) irrigation, the
transpiration will depend on the spatial distribution of the roots and
water in the soil, due to the reliance of root signals. It has been
shown that the concentration of xylem ABA from the roots of plants can
be described as a function of water potential of the dry and wet
zones. \citep{liu2008measurement,dodd2008accounting}. Several root
models suitable for PRD exists. E.g.\@~\cite{vrugt2001calibration}
proposes a flexible, empirical \twod{} root model with a radial
geometry, which \cite{apri} successfully used to model PRD irrigation
of a vineyard.  A static description of the root density based on
measurements was used in the later application.  For perennial plants
using a static description of the root system during the irrigation
season can make sense, but for annual plants, such as most vegetables,
the root system must be considered dynamic.  A dynamic root growth
model for row crops based on diffusion theory was proposed
...
field crop.  An example of the later would be the model found
...
density function $L\, (x,z)$, where the independent variables are
horizontal distance to the plant $x$ and vertical distance to the soil
surface $z$.  In contrast architectural root models track the
structure and branches of the roots.  See \cite{wang2004modelling} for
an overview of existing \oned{}, \twod{}, and \threed{} root models,
both parametric and architectural.

Radial coordinates are a natural choise for looking at the root zone
...
crops, based on on an extension of the model by \cite{gp74}.  The
assumption is that the roots are sufficiently dense in the direction
of the rows for us to ignore that dimension.  The two dimensions then
become depth and horizontal distance from the row.  The proposed model
is static in that it doesn't include root growth, but it can be
parametrized dynamically based on root mass and the depth and width of
the root zone.

The new root model was used in the SAFIR (SAFe IRrigation) EU project
\citep{ssoc,safir2010} as part of the Daisy agro-ecological model
\citep{daisy-fertilizer,daisy-ems,daisy2012} for calculation of water and solute
uptake \citep{daisyN}.  The SAFIR project had two aspects, the first
being assessment of the risk for human health with regard to use of
waster water for irrigation \citep{Forslund2010440,Surdyk2010451}, and
the second water saving irrigation techniques, in particular PRD
...
\oned{} to \twod{}, and used for modelling the
experiments.  The original Daisy crop model \citep{eupotato}
calculates root dry mass and root zone depth, and assumes a root
density distribution following the model by \cite{gp74}.  With the
\twod{} extension root zone width was added to the crop model, as well
as the root density distribution model discussed in this paper.
...
specified threshold $L_m$.  By inserting this in \MyEq{eq:g+p}, we
get
\begin{equation}
  L_m = L_0\, e^{-a \, d_c}
  \label{eq:root-depth}
...
$L_{0,0}$) is the root density in the top center of the row.  We then
...
dimensions. This is comparable to the approach taken in
\citet{pedersen2010modelling}.

To estimate values for the parameters $a_z$, $a_x$ and $L_{0,0}$, we
assume as before that the root depth and mass are known, and now
...
density at that depth will be below $L_m$.  Similarly, we define the
width $w_c$ as the horizontal distance from the row where the root
density at the surface ($z = 0$) is equal to $L_m$.
\begin{equation}
  L_m = L\, (0,d_c) = L\,(w_c,0)
  \label{eq:minroot}
\end{equation}
...
$a_z$~\MyEq{eq:Qaz}, $a_x$~\MyEq{eq:aztoax}, and
...
Beijing.  Potatoes (\textit{Solanum tuberosum} L. cv. 'Zheng Zha No.5')
were grown at the CAAS site, while the CAU site were growing tomatoes
(\textit{Solanum lycopersicum} L. cv. 'Hong-fen').

\subsubsection{Treatments}

The SAFIR project had two main aspects. The first aspect was the reuse
of waste water for irrigation, with emphasis on human safety.  All
treatments were fully fertilized, and the constituents of the waste
water were unlikely to harm crop growth.  Hence, for crop growth the
...
2008.  Unfortunately, the 2006 harvest failed, so we discarded that
data set.  All treatments were done with three replicates, but due to
the labour intensive nature of root density sampling, that was only
performed on selected plots as outlined in
table~\ref{tab:caas2008treatments}. At CAU, root density sampling was
performed in 2007 and 2008 at selected plots, see
...
\begin{table}[htbp]
  \caption{CAAS 2008 treatments with root density measurements.  
    ...
    metals (HM), or ultraviolet light (UV). \citep{battilani2010decentralised}}
...
Both sites used ridge systems.  To describe it, we used a coordinate
...
illustrated on figure~\ref{fig:sample-caas}.  At the CAU site, the
sampling was done in a cross shape, with five sample locations at the
...
\texttt{MathiasN.Andersen@agrsci.dk} to obtain a password for the
database.
...
figure~\ref{fig:sample-cau}.  For each year, we have one dataset for each
plot, plus one additional dataset consisting of all measurements from
...
described in section~\ref{sec:model-1d}.  For the \twod{} model, we
created a similar function that in addition took the horizontal root
zone width ($w_c$) as a parameter, based on section~\ref{sec:model-2d}
.  For each dataset and model, we found the parameter values that gave
the highest R$^2$ using the~\cite{nelder1965simplex} simplex
algorithm.
...
the \twod{} model provides no significant advantage over the \oned{}
model.  We used $p=0.05$ as the test criteria.
...
is placed in the top of the ridge.  We also ignore that the ridge
...
both the horizontal and vertical variation in root density.  Based on
