...
\item Drop table~\ref{tab:caas-phenology} and~\ref{tab:cau-phenology}? (Phenology)
\end{itemize}
...
\item TODO: Modellering af PRD kræver jordfysisk beskrivelse af vand
  bevægelse i 2 dimensioner, samt en \twod{} model for rodtæthed (fokus for
  denne artikel).
\item TODO: Om SAFIR \cite{safir2010}.
\item TODO: Om Daisy. Udvikling af \twod{}: \cite{1dvs2d}.  Brug i PRD: \cite{ssoc}
\item TODO: State-of-the-art rodmodellering.  Motivation for GP2D.
\end{itemize}

TODO: PRD.  At least \twod{}due to dry and wet zone.  Water move from wet to
dry zone.  Need's Richard's equation.  For looking at the root zone of
a single crop, radial coordinates can be used.
E.g. \cite{vrugt2001calibration}, \cite{apri} (PRD). Problem for
overlapping root zones.  In that case, simplification: Row crops.

...

Partial root zone drying is an irrigation technique where irrigation
is applied on only part of the root zone, dividing it into a wet and a
dry zone.  The idea is then that root signals from the dry zone will
close lower the hydraulic conductivity through of the stomata, and
thus reduce the transpiration from the wet zone.  For this to work,
the irrigation scheme must alternate between what part of the root
zone is wet, and what part is kept dry.  User properly, the technique
has been showned to provide significant water savings (TODO: REF).
...
subsurface drip irrigation and furrow irrigation.  The irrigation
...
zone by the model), so we have discarded that data set.  All
...
from the same site, in effect a dataset where we considered the
...
is always above 0.55, with a median of 0.86.  The potato (CAAS)
tend to have a wider root zone and more root mass than the tomato
(CAU), but no clear difference in root zone depth.  The 2008 CAU
sampling also shows the smallest deviation between plots.  The \oned{}
R$^2$ were much lower (less than 0.20) for the plots where the
estimated \twod{} root zone radius was less than half the distance
between rows.  The F-test showed that the \twod{} model gave a
significant better fit for all the aggregate datasets, half the CAAS
...
represent both irrigation methods and both irrigation strategies.
However, the two 2008 sampling at CAU shows a tendency of the roots to
concentrate near the center of the row, as illustrated on
