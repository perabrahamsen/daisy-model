...
\author{Per Abrahamsen\attmark{a,}\footnote{Corresponding author.
    ...
    Frederiksberg.  Email: pa@life.ku.dk} \and 
  Mathias Neumann Andersen\attmark{b} \and Søren Hansen\attmark{a} \and
  Guitong Li\attmark{d} \and Mikkel Mollerup\attmark{a} \and Xuebin Qi\attmark{c}\\
  \\
...
\begin{abstract}
  A \twod{} model of water movement in the soil is needed to predict
  how to most efficiently apply irrigation in order to achieve partial
  root zone drying.  This model must include a \twod{} root model.  In
  this paper we extend an empirical root density distribution based on
  densely populated homogeneous fields (\oned{}) to row crops
  (\twod{}).  The row crops are modeled as having a uniform density in
  the direction parallel to the rows, but variable in the plane
  perpendicular to the row.  In each case we demonstrate how to find
  the distribution parameters from the root dry matter and the size of
  the root zone.
...
With PRD irrigation, the transpiration will depend on the spatial
distribution, due to the reliance of root signals.  This makes the
spatial distribution even more important.  Several models
incorporating this exists, e.g.\@~\cite{vrugt2001calibration} proposes
a flexible, empirical \twod{} root model with a radial geometry, which
\cite{apri} successfully use to model PRD irrigation of a vineyard.  A
static description of the root density based on measurements was used
in the later application.  For perennial plants using a static
description of the root zone during the irrigation season can make
sense, but for annual plants, such as most vegetables, the root zone
must be considered dynamic.  A dynamic root growth model for row crops
based on diffusion theory was proposed
...
definition of the Lambert-W function \citep{lambert58,euler83}.  The
expression on the left hand side of the equation is depicted on
figure~\ref{fig:W}.
\begin{figure}[htbp]
  \input{rootdens_W}
  ...
  \label{fig:W}
\end{figure}
After finding a value for $W$, we can find the desired density
...
$-a_z d_c$ is exactly at the top point ($2^2 e^{-2}$),
...
\begin{table}[htbp]
  \caption{CAAS model fit.  The best fit for root zone depth ($d_c$),
    ...
    the \twod{} model provides a significantly better fit (\textbf{bold}).} 
...
$L^\prime (z) $ & L/L$^3$ & Soil limited root density at soil depth $z$\\ 
$L\, (x,z)$& L/L$^3$ & Root density at soil depth $z$ and distance $x$ from row\\
$L^* (x,z)$& L/L$^3$ & Root density from multiple rows at position $(x,z)$\\
$L^* (z)$& L/L$^3$ & Average vertical root density from multiple rows at depth $z$\\
...
\item Are the \cite{richardseq}, \cite{nelder1965simplex},
  \cite{euler83}, and \cite{lambert58} citations too silly?
\end{itemize}
