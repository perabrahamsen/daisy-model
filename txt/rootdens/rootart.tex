...
\begin{abstract}
  ...
  distribution is extended to two dimensions, and the parameters are
  ...
  (\textit{Solanum lycopersicum} L.) are measured at two sites.  Both
  the original and the extended model are fitted to these datasets. An
  F-test is used to determine whether the extended model is
  significantly better than the original model.

  \emph{Results} The extended model is superior when the root zone
  ...
  \emph{Conclusions} We are able to adequately describe the measured
...
With alternate partial root zone drying (PRD) irrigation, the
...
geometry, which \cite{apri} successfully use to model PRD irrigation
...
the root system must be considered dynamic.  A dynamic root growth model
...
a good choise.  But when the root zones of the plants overlap, the
...
assumes a root density distribution following the model by
\cite{gp74}.  With the \twod{} extension root zone width was added to
the crop model, as well as the root density distribution model
...
define the following root distribution
\begin{equation}
...
The distribution in \MyEq{eq:g+p} implies a gradual decrease of roots
...
further down.  To handle this, we can divide the root depth into a
crop specific and soil independent potential root depth $d_c$, and
soil specific and crop independent maximum root depth $d_s$.  The
actual root depth $d_a$ is then the shallowest of these two.
\begin{equation}
  d_a = \min (d_c, d_s)
...
In order to solve \MyEq{eq:Lambert} with regard to $W$, we first
...
to~\MyEq{eq:Lambert} when $-L_m \frac{d_c}{l_r}$ is exactly at the
...
\item The solution for $W < -1$ represents a large $a$ parameter. From
  \MyEq{eq:L0-found} we see this also means $L_0$ is large.  Thus,
...
\MyEq{eq:root-integral2} are only valid if $a_z > 0$. Since
