...
of root signals. It has been shown that the concentration of xylem ABA
from the roots of plants can be described as a function of water
potential of the dry and wet
...
using a static description of the root zone during the irrigation
season can make sense, but for annual plants, such as most vegetables,
the root zone must be considered dynamic.  A dynamic root growth model
...
complex and results in long simulation times.  Hence, finding a
\twod{} approximation is therefore desirable.  In this paper we
therefore propose a simple \twod{} model with Euclidean coordinates
for row crops, based on on an extension of the model by \cite{gp74}.
The assumption is that the row is sufficiently dense in the direction
...
the crop model, as well as the root zone density distribution model
...
system where $L\, (0,0)$ (also denoted $L_{0,0}$) is the root density in the
...
\subsubsection{Estimating the parameter values}
...
zone width ($w_c$) as a parameter.  For each dataset, we found the
...
F-test value.  The horizontal root zone width is the parameter that
...
width was less than half the distance between rows.  The F-test
...
and all but three of the CAU datasets.
