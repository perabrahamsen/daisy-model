...
\begin{abstract}
  ...
  In this paper we extend an empirical root density distribution based
  on densely populated homogeneous fields to row crops.  The row crops
  are modeled as having a uniform density in the direction parallel to
  the rows, but variable in the direction perpendicular to the row.
...
below $L_m$.  Similarly, we define the radius $w_c$ as the horizontal
...
$L_z^*$ & L/L$^3$ & Soil limited root density at soil depth $z$\\ 
