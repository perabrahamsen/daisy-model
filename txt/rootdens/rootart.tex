...
\begin{table}[htbp]
  \caption{CAAS model fit.  The best fit for root zone depth ($d_c$),
    root zone radius ($w_c$), and total root dry matter ($M_r$) is show
    for each plot.  The `All' plot indicate a dataset containing alle
    ...
    the \twod{} model provides a signficantly better fit.} 
\label{tab:caas-ftest}

\begin{tabular}{rrrrrrrrr}\hline
Plot	& $d_c$ [cm]	& $w_c$ [cm]	& $M_r$ [Mg/ha]	& 2D R$^2$	& 1D R$^2$	& F	& F (0.05)\\\hline
...
\begin{table}[htbp]
  \caption{CAU model fit.  The best fit for root zone depth ($d_c$),
    ...
    the individual plots.  Furthermore, the R$^2$ for both the \oned{}
    and \twod{} fits are listed, as well as an F test indicating whether
    the \twod{} model providesa signficantly better fit.} 
  \label{tab:cau-ftest}
