...
number style from the control panel (Start $\rarrow$ Control panel
$\rarrow$ International).
...
(defprogram test gnuplot
  ...
  (defprogram main batch
    (run (test)
         (test (where "myplot.eps"))))
  
  (run main)
\end{verbatim}  
\begin{itemize}
\item This example chooses another command file (\texttt{kurt.gnuplot}
  instead of \texttt{daisy.gnuplot}).
\item It selects the range for the x-axis (begin and end) and both y-axes
  (ymin, ymax, y2min, y2max) manually, instead of letting gnuplot
  choose its own.
\item We give the plot a title, ``gnuplot demonstration''.
\item We ask for a plot with the default width, but only half the
  default height.
...
\item We create a `\texttt{batch}' program that run the plot twice.
  Once to a window, and once to a file name `\texttt{myplot.eps}'.
  Daisy deduce the desired format from the file name.  Both are
  created from the same `\texttt{kurt.gnuplot}' command file.  You can
  create as many plots as desired from the same gnuplot command file.
\end{itemize}
