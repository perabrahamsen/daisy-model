...
research community, leading to two attempts to create a graphical user
interfaces.  The first was a standalone interface written in Delphi by
a pair of student programmers, and the second was a integrated GIS
version written by DHI.  The Delphi version never quite got to the
point where we were ready to distribute it, before the the funding was
used up.  A graphical log file viewer did for a fast presentation of
the output did came out of the project.  The GIS version is a current
commercial product, but some frictions do occur when people meet the
limitations of the GIS interface, and want to use features from the
research version.  Since the GIS interface tend to be build upon older
versions of Daisy, mixing features from the latest research version
can meet incompatibilities.
