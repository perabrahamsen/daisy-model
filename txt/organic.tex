...
the pools have such a dk:fuldt optrukken outgoing line.  The amount of
carbon being eaten is directly propertional with the size of the pool.
We call this the \emph{turnover rate}.  The number is the
corresponding halftime.  The \textsc{som1} pool thus have a halftime
of 703 years, meaning if there was no input it would be half the
original size after 703 years.  For some pools this time is given in
days, e.g.\ the \textsc{smb2} pool has a turnover halftime of 69 days.
  
If we follow the line from \textsc{som1} it split into two parts.
...
\textsc{som1}.  The biomass pools has a turnover rate like the other
pools, but also a maintenance rate.  This is the price of staying
alive, and is indicated by a stippled line out of the \textsc{smb}
pools.  The rates and even the number of \textsc{aom} pools is
variable, the rates given here is just examples.  Also, there can be
...
The halftimes given coresponds to 0\% clay, 10$^\circ$C and field
capacity (pF 2).  The halftimes will be longer with more clay, lower
temperature, og dryer soil.  They will be shorter with higher
temperature.
...
We used \daisy{} to simulate the effect on soil organic
