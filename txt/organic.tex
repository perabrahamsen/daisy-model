...
\begin{verbatim}
SH: check værdier, specielt smb.  Er 50\% efficiency ok med sander (i
den ny model)?
\end{verbatim}
...
carbon being eaten is directly propertional with the size of the
pool.  We call this the \emph{turnover rate}.  The number is the
...
days, e.g.\ the \textsc{smb2} pool has a turnover halftime of 69
days.  
...
The biomass pools has a turnover rate like the other pools, but also a
maintenance rate.  This is the price of staying alive, and is
indicated by a stippled line out of the \textsc{smb} pools.  The rates
and even number of \textsc{aom} pools
