...
The current \daisy{} model calibration is based on agriculutural
topsoil, where all the humus is active.  This means the calibration is
not good for subsoils, nor for some types of humus-rich topsoils.  The
\textsc{som3} pool you last saw in figure~\ref{fig:om2} exists to
compensate for these shortcommings.  The \textsc{som3} pool is inert,
so for the soil organic matter system it is as if it didn't exist.  It
still affects other aspects of \daisy{}, such as the hydraulic
adsorption properties of the soil.  

For the subsoil, we by default assume the system is in equilibrium
with the input, and park enough humus in the \textsc{som3} so that
this assumption holds true.  Yes, we earlier argued that the
equilibrium assumption is wrong.  It still is, also for the subsoil.
Hwoever, the subsoil contribute relatively little to the total system,
so we can get away with this assumption.  For topsoil, the user can
explicitly park some of the humus in the \textsc{som3} pool, if he has
reason to believe it to be necessary.
...
a change in input level.  This is because the \textsc{som2} pool will
not yeat have had any chance to adopt to the new input level.  With
the current model, the \textsc{som2} pool needs several decades to
catch up with changes in input.  This means the quasi-equilibrium
assumption is still suspect in a modern agrocultural country like
...
introduced.  Out long term simulations has shown that the
...
intitialization will be redone\footnote{More precisely, if the
...
fluctates with rotation, season and climate, the simulated
\textsc{som1} fraction stay stable.  This is because most of the minor
fluctations happens in the \textsc{aom} and \textsc{smb} pools, the
slower \textsc{som2} does not have time to adapt.  This also explains
the relatively noise on the initialisation points, the
