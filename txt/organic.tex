...
The first such experimental change that made it back into \daisy{} wasmade by Torsten Müller (see XXX 19??), who adjusted the turnover rates
...
the SOM3 pool will be explained later.
...
focus on the effect of clay (see Biomod/Bjørn/AP), including
phosphorous and calibration for forrest soil.  Our conclusion is that
model has changed in the past, and will continue to change in the
future. 
...
for changed farm practice at an individual farm level.  For the last
case, \daisy{} has also been used by consulting agencies on behalf of
the farmers, who seek permission to change farming practice.

The effect of this increased use has been that we no longer can rely
on the users being experts.  Therefore, a project was
started in order to provide standardized procedures for setting
\daisy{} simulations (see XXX).   As part of this components, most
components of \daisy{} was evaluated in order to find parameters and
submodels who could be initialized automatically from default values,
or from other parameters.  These parameters and submodels are still
available to the expert user, but will be hidden for the user who do
not have the expertice or data available to change.  The result is
that the model now appear much simpler to the user, even if the
substantial changes to the models have been made.
...
to readjust the partitioning between \sc{som1} and \sc{som2}, there
was a strong tendency that users just picked the equilibrium values.
Unfortunately, soil organic dynamics is not one of the cases where
equilibrium is an good assumption.  The quality of the soil in general
adjust slower than both the changes in farming practice, and the
warmup periods used for such simulations.  Thus, in the common case
the initial state wil \emph{not} be an equilibrium, and the result of
the simulation \emph{will} be dominated by the choice of initial
state.   
...
Danish conditions high clay content (10 \percent{}).
Figure~\ref{fig:clayration} shows the \sc{som1}/\sc{som2} ratio, which
is can be used for initializing \daisy{}.  Finally,
...
FIG{fig:clayratio}: Long term simulation of \sc{som1}/\sc{som2} ratio
from changing from high to low input on a clayish loam.

FIG{fig:sandratio}: Long term simulation of \sc{som1}/\sc{som2} ratio
...
assume that no farming land is in equilibrium.  At such a time scale,
not only farming practice but also climate is going to change.
