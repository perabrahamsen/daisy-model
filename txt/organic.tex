...
The factors $a$ to $t$ depend on the base calibration of the model, so
the factors are different for the original calibration
(figure~\ref{fig:om1}) and the Sander calibration
(figure~\ref{fig:om2}).  They also depend on the clay content of the
soil, the temperature, and the water potential.  
...
The description is also robust with regard to the number of pools,
adding a new pool will just add another equation, and another term in
all the existing equations.  The general form is:
\begin{eqnarray}
  \Delta\mbox{\textsc{smb}}i &=& 
  \Sigma_{k=1}^{\mbox{\#\textsc{smb}}} a_{i,k}\mbox{\textsc{smb}}k + 
  \Sigma_{l=1}^{\mbox{\#\textsc{som}}} b_{i,l}\mbox{\textsc{som}}l +
  c_i\, \Delta\mbox{\textsc{aom}} 
  \hspace{1cm};i = 1\ldots\mbox{\#\textsc{smb}}\nonumber\\
  \Delta\mbox{\textsc{som}}j &=& 
  \Sigma_{k=1}^{\mbox{\#\textsc{smb}}} d_{j,k}\> \mbox{\textsc{smb}}k + 
  \Sigma_{l=1}^{\mbox{\#\textsc{som}}} e_{j,l}\> \mbox{\textsc{som}}l +
  f_j\, \Delta\mbox{\textsc{aom}} 
  \hspace{1cm};j = 1\ldots\mbox{\#\textsc{som}}\nonumber
\end{eqnarray}



\begin{verbatim}
SH: Genereliseret udtryk? 
SH: dSMBi = Sigma (a_i,j * SMBj) + Sigma (b_i,k * SOMk) + c_i dAOM
SH: dSOMi = Sigma (d_i,j * SMBj) + Sigma (e_i,k * SOMk) + f_i dAOM
\end{verbatim}

The reason we have included all the factors are 
